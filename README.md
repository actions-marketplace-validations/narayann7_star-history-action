...
**Fonts:** GitHub strips `@font-face` from SVGs it serves through `<img>`/`<picture>`, so a custom font renders only in the PNG output (`readme-format: png`), not the SVG. Set `font-family` to any Google Fonts name to restyle the PNG. The action downloads the font and reads its real internal name so it renders even when that differs from the family string. If the download fails, it falls back to Comic Neue and the run still succeeds. The SVG's legend box and title are laid out from text widths measured in the PNG font and pinned with SVG `textLength`, so whatever font a browser substitutes is fitted into the same space rather than overflowing the legend. To keep that fitting mild, the SVG asks for a fallback stack close to Comic Neue in width (`Chalkboard SE` on macOS, `Comic Sans MS` on Windows, then `sans-serif`); a browser whose default font is monospace would otherwise have its text compressed by about a quarter. Non-Latin families (e.g. `Noto Sans SC`) work too, though single-weight fonts render the title in the regular weight since no bold face exists.
